...
struct DeviceStorageInsight: SmartInsightProviding {
    ...
    init(freeDiskSpace: ByteCountFormatter.Units.Bytes? = Device.freeDiskSpaceInBytes,
         totalDiskSpace: ByteCountFormatter.Units.GigaBytes = Device.totalDiskSpace) {
        if let freeDiskSpace {
            ...
            result = .error(message: "Free storage insight not avaiable")
        }
    }
}
